/* eslint-disable */
/* Pentecom — Consulting page
   ============================================================
   Visual motif: technical specimens. Every post-hero section gets
   visual specimens that demonstrate the work — hairline-bordered
   document fragments rendered in the same mono/Geist language as
   the rest of the brand. Specimens are abstracted, never literal
   client material.

   ============================================================
   IMPORTANT — TERMINOLOGY (do not conflate)
   ============================================================
   CAPABILITIES = things we can DO (verbs / abilities / services
                  we perform — e.g. "establish applicability
                  models," "manage sustainment relationships").
   SOLUTIONS    = packaged OFFERINGS we deliver (named products /
                  programs — e.g. Consulting, Technical Data
                  Services, etc).
   These are distinct concepts. Never use "Capabilities" as a
   label for a solutions list, and never treat the two as the
   same section.
   ============================================================

   Sections:
     1. ConsultingHero        editorial cover + iso-cube artifact
     2. ConsultingTrust       leadership statement + intersection constellation
     3. ConsultingExpertise   2 document-index feature cards + 7 equal-weight tile cards
     4. ConsultingWhyExperts  paragraph + tilted publication stack
     5. ConsultingHowWeHelp   4 capability cards w/ per-card specimens (capabilities ≠ solutions)
     6. ConsultingServices    3 service cards (long-term retainer / training / kickoff mentoring)
     7. ConsultingCta         CTA + side artifact
     8. ConsultingContacts    contact cards
*/

const { useState, useEffect, useRef } = React;

const useInViewC = () => {
  const fn = (typeof window !== "undefined" && window.useInView3);
...
const ConsultingCta = () => {
  useInViewC();
  const Panels = (typeof window !== "undefined" && window.FloatingPanelsArtifact) || null;
  return (
...
/* =========================================================================
   ...
   ========================================================================= */
const CONSULTING_TWEAK_DEFAULTS = /*EDITMODE-BEGIN*/{}/*EDITMODE-END*/;

const ConsultingApp = () => {
  const Header = window.Header3;
  const Footer = window.Footer3;
  return (
    <React.Fragment>
...
ReactDOM.createRoot(document.getElementById("root")).render(<ConsultingApp />);
